...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3"
        ...
        <x-page-title title="User" pagetitle="Add User Details" />
                ...
                <div class="card-body p-4">
                    <form action="{{ route('users.store') }}" method="POST">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-9">
                                <input type="text" class="form-control" id="input35" name="yourname" placeholder="Enter Your Name" required>
                                @error('yourname')
                            ...
                            <div class="col-sm-9">
                                <select class="form-select" id="inputCountryCode" name="code" required>
                                    ...
                                    @foreach($countryCodes as $key => $value)
                                        <option value="{{ $key }}" 
                                            @if($key === $user_countryCode['country_code']) selected @endif>
                                            {{ $value }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-9">
                                <input type="number" class="form-control" id="input36" name="phone" placeholder="Phone No" required>
                                @error('phone')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-9">
                                <input type="password" class="form-control" id="input38a" name="password" placeholder="Choose Password" required>
                                @error('password')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
